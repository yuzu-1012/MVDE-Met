...
def info_nce(query, positive_key, negative_keys=None, temperature=0.1, reduction='mean', negative_mode='unpaired', metric='cosine'):
    ...
    if negative_keys is not None:
        ...
        # logits = negative_keys
        # labels = torch.zeros_like(negative_keys, device=query.device)
    else:
        ...
        logits = torch.mean(sim_matrix, dim=1, keepdim=True)
        labels = torch.ones_like(logits, device=query.device)
